(() => {
  ...
  function renderDialog(detail, report, updateUrl = true) {
    ...
    const legacyIssue = Array.isArray(game.legacyIssues) ? game.legacyIssues[0] : null;
    if (legacyIssue?.url) {
      const archived = document.createElement('a'); archived.className = 'text-link archived-issue-link'; archived.href = legacyIssue.url; archived.target = '_blank'; archived.rel = 'noreferrer'; archived.textContent = 'Migrated from archived discussion ↗';
