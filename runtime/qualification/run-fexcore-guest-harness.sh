...
require_equal() {
  ...
  if [[ "$actual" != "$expected" ]]; then
    echo "FEX Phase 1 runner: $label did not match expected value" >&2
    exit 1
  fi
}
...
timeout 45s "$adb_bin" -s "$serial" shell am instrument -w -r \
  -e class com.bachatas4.android.FexGuestHarnessDeviceTest \
  com.bachatas4.android.test/androidx.test.runner.AndroidJUnitRunner >"$run_directory/instrumentation.raw" 2>&1
instrumentation_status=$?
set -e
finished_ns=$(date +%s%N)
"$adb_bin" -s "$serial" logcat -d -T "$logcat_start" -s BachataFexGuestHarness:I '*:S' >"$run_directory/logcat.raw"
...
node "$project_root/runtime/qualification/create-fex-phase1-evidence.mjs" \
  "$duration_ms" "$run_directory/instrumentation.raw" "$run_directory/logcat.raw" "$evidence"
node "$project_root/runtime/tests/verify-fex-phase1-evidence.mjs" "$evidence"
